...
// Function to calculate the modular exponentiation (base^exp mod modulus)
...
void diffie_hellman() {
    ...
    int secret_key_a = mod_exp(public_key_b, private_key_a, p);
    int secret_key_b = mod_exp(public_key_a, private_key_b, p);

    // Verify that both parties have the same shared secret key
    if (secret_key_a == secret_key_b)
        std::cout << "Shared secret key: " << secret_key_a << std::endl;
    else
